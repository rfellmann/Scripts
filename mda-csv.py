...
IP_RANGES_BASE_URL = 'https://rfelab.eu2.portal.cloudappsecurity.com/api/v1/subnet/'
...
def get_records():
    ...
    while has_next:
        response = requests.post(
            IP_RANGES_BASE_URL, json=list_request_data, headers=HEADERS)
        if response.status_code != http.HTTPStatus.OK:
            raise Exception(
                f'Error getting existing subnets from tenant. Stopping script run. Error: {response.content}')
...
def rule_matching(record, ip_address_ranges, category, tag, isp_name, ):
    ...
    existing_tags = sorted([existing_tag.get('id')
                           for existing_tag in record.get('tags', [])])
...
def create_update_rule(name, ip_address_ranges, category, tag, isp_name, records, request_data):
    for record in records:
        ...
        if record["name"] == name:
            ...
            if not rule_matching(record, ip_address_ranges, category, tag, isp_name):
                ...
                response = requests.post(
                    f"{IP_RANGES_BASE_URL}{record['_id']}/{IP_RANGES_UPDATE_SUFFIX}", json=request_data, headers=HEADERS)
                if response.status_code == http.HTTPStatus.OK:
                    print('Rule updated', request_data)
                else:
                    print('Error updating rule. Request data:',
    ...
    if response.status_code == http.HTTPStatus.OK:
        ...
        print('Error creating rule. Request data:',
            request_data, ' Response:', response.content)
        # added record
        return record
            ...
            if record_id:
                for record in records:
                    ...
                    if OPTION_DELETE_ENABLED:
                        ...
                        for record in records:
                            requests.delete(
                                f"{IP_RANGES_BASE_URL}{record['_id']}/", headers=HEADERS)
